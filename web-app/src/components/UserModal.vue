<template>
  <div class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
      <h2 class="text-xl font-bold mb-4">{{ user ? "Edit User" : "Add New User" }}</h2>
      <form @submit.prevent="saveUser">
        <div class="mb-4">
          <label class="block text-gray-700">Name</label>
          <input v-model="formData.name" type="text" class="w-full border rounded p-2">
        </div>
        <div class="mb-4">
          <label class="block text-gray-700">Surname</label>
          <input v-model="formData.surname" type="text" class="w-full border rounded p-2">
        </div>
        <div class="mb-4">
          <label class="block text-gray-700">Email</label>
          <input v-model="formData.email" type="email" class="w-full border rounded p-2">
        </div>
        <div class="mb-4">
          <label class="block text-gray-700">Role</label>
          <select v-model="formData.role" class="w-full border rounded p-2">
            <option value="caregiver">Caregiver</option>
            <option value="veterinarian">Veterinarian</option>
            <option value="volunteer">Volunteer</option>
          </select>
        </div>
        <div class="mb-4">
          <label class="block text-gray-700">Password</label>
          <input v-model="formData.password" type="password" class="w-full border rounded p-2">
        </div>

        <!-- Action Buttons -->
        <div class="flex justify-end gap-4">
          <button type="button" @click="$emit('close')" class="bg-gray-500 text-white py-2 px-4 rounded-lg hover:bg-gray-600">
            Cancel
          </button>
          <button type="submit" class="bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600">
            {{ user ? "Update" : "Create" }}
          </button>
        </div>
      </form>
    </div>
  </div>
</template>

<script>
import axios from 'axios';

export default {
  props: {
    user: {
      type: Object,
      default: null
    }
  },
  data() {
    return {
      formData: {
        name: "",
        surname: "",
        email: "",
        role: "volunteer",
        password: ""
      }
    };
  },
  watch: {
    user(newVal) {
      if (newVal) {
        // Populate form with user data for editing
        this.formData = { ...newVal, password: "" };
      } else {
        // Clear form for creating a new user
        this.formData = {
          name: "",
          surname: "",
          email: "",
          role: "volunteer",
          password: ""
        };
      }
    }
  },
  methods: {
    async saveUser() {
      try {
        if (this.user) {
          // Update existing user
          await axios.put(`${process.env.VUE_APP_BACKEND_URL}/users/${this.user.id}`, this.formData);
        } else {
          // Create new user
          await axios.post(`${process.env.VUE_APP_BACKEND_URL}/users`, this.formData);
        }
        this.$emit("saveUser");
        this.$emit("close");
      } catch (error) {
        console.error("Error saving user:", error);
      }
    }
  }
};
</script>

<style scoped>
/* Modal specific styles */
</style>
